...
brand_colors = {
    ...
    'dark_bg': '#2C3E50'        # Dark Background for dark mode
}
...
def get_crypto_data(symbol='BTC/USD', limit_hours=168):
    ...
    df = bars[['close']].rename(columns={'close': 'Price'})
    return df
...
app.layout = dbc.Container([
    ...
    dbc.Row(dbc.Col([
        html.H4("Open Position"), html.Div(id='position-table')
    ]), className='mt-4')
], fluid=True)
...
def update_all(n, qty, tp, sl, strategy, rsi_threshold):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df['Price'], mode='lines', line=dict(color=brand_colors['primary']), name='Price'))
    ...
    if not trades.empty:
        for _, t in trades.iterrows():
            ...
            fig.add_trace(go.Scatter(x=[t['exit_time']], y=[t['exit_price']], mode='markers', marker=dict(symbol='triangle-up', size=10, color=brand_colors['secondary']), name='Exit'))
    fig.update_layout(paper_bgcolor=brand_colors['background'], plot_bgcolor=brand_colors['background'], title="BTC/USD Price with Trades")

    # P&L Chart
    pnl_fig = go.Figure()
    if not trades.empty:
        pnl = ((trades['entry_price'] - trades['exit_price']) / trades['entry_price']).cumsum()
        pnl_fig.add_trace(go.Scatter(x=trades['exit_time'], y=pnl, mode='lines+markers', name='Cumulative P&L', line=dict(color=brand_colors['secondary'])))
    pnl_fig.update_layout(title='Cumulative P&L', paper_bgcolor=brand_colors['background'], plot_bgcolor=brand_colors['background'])
    ...
    dd_fig.update_layout(title='Drawdown', paper_bgcolor=brand_colors['background'], plot_bgcolor=brand_colors['background'])

    # Position Table
    try:
        pos = api.get_position('BTC/USD')
        pos_df = pd.DataFrame([{
            'Symbol': pos.symbol, 'Qty': pos.qty, 'Market Value': f"${float(pos.market_value):,.2f}",
            'Unrealized P/L': f"${float(pos.unrealized_pl):,.2f}", 'Side': pos.side
        }])
    except:
