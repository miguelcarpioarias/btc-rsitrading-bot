...
from oandapyV20.contrib.requests import TradeCandleStream, TradeCandleRequest
from oandapyV20.contrib.requests import OpenPositions
...
def get_candles(count=3):
    # Fetch last `count` 15-min candles
...
app.layout = dbc.Container([
    dbc.Row(dbc.Col(html.H2('EUR/USD Pattern Bot'), width=12), align='center'),
    dbc.Row([
        dbc.Col(dbc.Card([dbc.CardHeader('Settings'), dbc.CardBody([
            ...
            html.Br(),dbc.Button('Start Bot',id='start-btn',color='secondary')])]),width=4),
...
def update_dash(n, qty, tp, sl, strategy, rsi_threshold):
    ...
    if not trades_df.empty:
        ...
        for idx,row in trades_df.iterrows():
            entry,exit = row.entry_price, forex_df.Price.iloc[-1]
            ret = ((entry-exit)/entry) if row.side=='Pattern' and settings['strategy']=='Pattern' else 0
            pnl.append(ret)
