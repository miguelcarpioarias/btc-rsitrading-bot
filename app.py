...
def execute_trade(side, settings):
    ...
    trades_df.loc[len(trades_df)] = [datetime.now().astimezone(eastern), price, side, settings['pair']]
...
def auto_trade_loop(settings):
    global forex_df
    while True:
        forex_df = get_candles(settings['pair'])
...
def update_dash(n, n_int, pair, qty, tp, sl, strat, sf, ss):
    global trade_thread
    settings = {'pair':pair, 'qty':qty, 'tp':tp/100, 'sl':sl/100,
                'strategy':strat, 'sma_fast':sf, 'sma_slow':ss}
    if n and trade_thread is None:
        threading.Thread(target=auto_trade_loop, args=(settings,), daemon=True).start()
        trade_thread = True
    df = forex_df.copy()
    ...
    for _, r in trades_df[trades_df.pair==pair].iterrows():
    ...
    for p in pos_resp.get('positions', []):
        if p['instrument']==pair:
            ...
            pos_data.append({'Instrument':pair, 'Units':units, 'Unrealized P/L':f"${unreal:.2f}"})
